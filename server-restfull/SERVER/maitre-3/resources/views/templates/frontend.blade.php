       ...
       <!--header-->
        ...
        <div class="row">
          <div class="inner text-center">
            <h1 class="logo-name">@yield('caption')</h1>
            <h2>@yield('caption-header')</h2>
            <p>@yield('sub-caption')</p>
          </div>
        </div>
        </div>
      </div>-->
     ...
     <header id="header">
        <div class="container">
            <div id="mySidenav" class="sidenav">
              ...
              <a href="{{route('fr-home')}}"><i class="fa fa-list-alt" aria-hidden="true"></i> Menu</a>
              @if( DB::table('table')->where('ip',$_SERVER['REMOTE_ADDR'])->first() )
              <a href="{{route('fr-mycommand')}}">  
              @if($token)
                @if( DB::table('command')->where('table_token',$token)->where('status','<',5)->first() )
                    <span class="label label-warning">{{count( DB::table('command')->where('table_token',$token)->where('status','<',5)->get() )}}</span>
                    Minha Comanda
                @else
                    &nbsp;<i class="fa fa-bars" aria-hidden="true"></i> Minha Comanda
                @endif                
              @endif
              </a>
              @else
                <i class="fa fa-bars" aria-hidden="true"></i> Você não possui uma mesa.
              @endif
              <a href="#"><i class="fa fa-bell-o" aria-hidden="true"></i> Garçon</a>
              <!--<a href="#contact">Book a table</a>-->
              @if(Auth::check())
                <a href="{{route('ds-home')}}"><i class="fa fa-line-chart" aria-hidden="true"></i> Dashboard</a>
              @else
                <a href="{{route('login')}}"><i class="fa fa-user-o" aria-hidden="true"></i> Login</a>
              @endif
            </div>
            <!-- Use any element to open the sidenav -->
            <span onclick="openNav()" class="pull-right menu-icon">
            @if($token)
                @if( DB::table('command')->where('table_token',$token)->where('status','<',5)->first() )
                    <span class="label label-warning">{{count( DB::table('command')->where('table_token',$token)->where('status','<',5)->get() )}}</span>
                @endif
